...
static void drawPauseScreen() {
    ...
    mainFont->drawString( translate( "pause_message1" ), 
                           messagePos, alignCenter );
        
    messagePos.y -= 1.25 * (viewHeight / 15);
    mainFont->drawString( translate( "pause_message2" ), 
                           messagePos, alignCenter );
...
void drawFrame( char inUpdate ) {    
    ...
    if( !firstDrawFrameCalled ) {
        ...
        char *moveKeyMapping = 
            SettingsManager::getStringSetting( translate( "upleftdownrightkeys" ) );
            // upLeftDownRightKeys
